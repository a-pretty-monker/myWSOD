...
class Generalized_RCNN(nn.Module):
    ...
    def forward(self, data, rois, edges, labels):
        if cfg.PYTORCH_VERSION_LESS_THAN_040:
            return self._forward(data, rois, edges, labels)
        else:
            with torch.set_grad_enabled(self.training):
                return self._forward(data, rois, edges, labels)

    def _forward(self, data, rois, edges, labels):
        im_data = data
        if self.training:
        ...
        edges = edges.data.cpu().numpy()[0]
        mil_score = self.Box_MIL_Outs(box_feat, torch.tensor(edges[:, 1:],dtype=torch.long ).t().contiguous())
